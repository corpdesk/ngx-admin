/**
 * this component is based on my-inte-ract which derive users from pals group
 * this pms-inte-ract derive users from project or task depending on the context
 * to contain conversation within a given context use pms-inte-ract.service.ts to 
 * 1. set PmsInteRactService::pubCtx to control how it saves 
...
@Component({
  ...
  templateUrl: './pms-inte-ract.component.html',
  styleUrls: ['./pms-inte-ract.component.scss'],
...
export class PmsInteRactComponent implements OnInit {
  ...
  constructor(
    ...
    public svPmsInteRact: PmsInteRactService,
  ...
  ngOnInit(): void {
    ...
    this.svSocket.listen('push-react').subscribe((data: any) => {
      console.log('PmsInteRactComponent/Push react received');
      console.log('data:', data);
      if (data.length > 0) {
        this.addReact(data[0]);
      }
    });
  }
  ...
  reloadPubsData(){
    const f: CdFilter[] = this.svPmsInteRact.pubFilter();
    const fExt: CdFilter[][] = this.svPmsInteRact.pubFilterExt(this.selectedGroup);
  ...
  addReact(react) {
    ...
    console.log('addReact(react)/p:', p);
    this.Pubs = p;
  }
  ...
  getAvatar(pub) {
    console.log('starting PmsInteRactComponent::getAvatar(pub)');
  ...
  getSelectedAssociation(e: any) {
    ...
    if (e.group_name == 'consumer_users') {
      ...
      this.svPmsInteRact.setPubCtx(null, null,e);
